...
**Searching** is an ubiquitous computing operation in many software
applications. Since _searching_ is usually just a component of some larger task
that an application needs to complete it has to be done efficiently.  In a
Computer Science course we often study a few algorithms that solve this problem
with different strategies, depending mainly on the data configuration of the
collection where the searching will be done. For instance, if we already know
that the data is sorted in a linear list we recommend the [**binary
search**](https://en.wikipedia.org/wiki/Binary_search), which is a very
efficient strategy since it takes advantage of the data organization and
progressively discards half of the current search space each time the algorithm
advances. On the other hand, if we know nothing about the data disposition
inside the list, we often do a  [**linear
search**](https://en.wikipedia.org/wiki/Linear_search), which needs to check
each element of the collection in the worst case.

This assignment is designed to help you understanding these algorithms (and some variations of them) in depth by having to code them to look for a target value in an array _sequence container_. You will also need to **experimentally** compare the performance of these algorithms by timing them under different circurnstances of implementation strategy, data size and organization. This assignment is organized as follows:
...
4. In the fourth part, you have to implement another searching algorithm for sorted data that uses a strategy similar to the binary search: **ternary search**.
5. In the last part, you need to experimentally identify the _order of growth_ for both linear, binary, and ternary searches, by collecting and analysing the time taken to complete several searching operations on collection with increasing length.

# 2. How to pass an array to a function

Before continuing with the exercise, let us first review somes ways of passing an array to a function. This will be useful later on, since the search functions receive an array of integers and the target value as arguments.

To illustrate the **three strategies** to pass an array to a function, we'll consider a simple example:
	
	Write a function receives an array as an argument and prints its contents to the standard output.
...
1. **A regular array and its length**: This is the easiest way of passing an
   array as argument to a function: we provide the array name (pointer) to the
function along with a variable with the array’s size. The downside of this
mechanism is that the function that receives the array must _process the entire
array_. Of course, you may limit the portion of the array the function will
process by passing on a size that is smaller than the entire array; in this case the function will consider
the subset starting at the beginning of the array until the defined size.
Regardless of the size provided, the important point is that _the subset always
starts from the beginning of the array_. Below we have a function that follows
this basic strategy.
```c++
...
Another _precondition_ applies to both the binary and ternary searches: the input collection of unique elements _must be sorted_. This means it is valid to pass an unsorted collection of elements to the linear search function, but the same does not apply to the binary and ternary search functions.
...
The second question is _"how do we report back to the client whether the search
was successful or not?"_ Well, in case the search is successful, the function
should return [the index of / a pointer to] the location where the _element is
found within the container_. If the search fails, the function should return
[the index of / a pointer to] _the first element just past_ the last element of
the search range. For instance, if the input array, say `A[]`, has size 10, the
search range corresponds to the entire array, and the search fails, the
function should return [the index of / a pointer to] the element located at
`A[10]`, i.e. the first element just past the last valid element, which is
`A[9]`.
...
In the latter case (processing the entire array), if you unintentionally try to dereference the `last` pointer you may trigger the classical [_segmentation fault_](https://en.wikipedia.org/wiki/Segmentation_fault).
...
1. **Define the Interval**: Start with an initial interval `[first, last)`, where you believe the target value might lie.

2. **Iterate**: Perform the following steps in a loop until the interval becomes empty or the target value is found:

    a. Divide the current interval `[first, last)` into three subintervals of (approximately) equal length:
     * Assuming `mid1 = first + (last-first)/3` and `mid2 = mid1 + (last-first)/3`
     * First subinterval: `[first, mid1)`
     * Second subinterval: `[mid1,mid2)`
     * Third subinterval: `[mid2, last)`

    b. Test midpoints:
     * If `A[mid1]` is equal to the target value, return `mid1` as the result.
     * If `A[mid2]` is equal to the target value, return `mid2` as the result.


    c. If target was not found at the midpoints, determine which subinterval to investigate.
     * If `target < A[mid1]`, then the target value may lie in the first subinterval. Update `last` accordingly.
     * If `target > A[mid2]`, then the target value may lie in the last subinterval. Update `first` accordingly.
     * Otherwise, the target value may lie in the middle subinterval. Update both `first` and `last` accordingly.

3. **Result**: The algorithm either returns because it has found the `target` at either one of the midpoints during the investigation phase or because the subinterval becomes empty.
In the former case, the algorithm returns de proper index, whereas in the latter case the algorithm should return the original `last` value, indicating searching failure.

## 3.4 Questions for pondering...
...
In this part of the assignment you must assume that the searching will be done
over a data collection that _might have repeated elements_. This
means a given value may appear in the collection multiple times.
...
Because we are interested in the rate of growth of each algorithm, you need to
progressively increase the size of the input array and store the running time
for that instance. Start with an array with $10^3$ elements and increase it
up to $10^{7}$ elements, adding 200 thousand elements each time you increase
...
3. _"Which one is the fastest, the **binary** or the **ternary** search algorithm?"_
Again, you must run both algorithms for the same input array, collect their running times and create a single graph with two curves: one for binary and other for ternary search results.
...
To successfully complete this assignment and receive full credits **(130 in total)** you should do the following tasks:

1. Implement and test a linear search function **(15 credits)**.
2. Implement and test a binary search function **(15 credits)**.
2. Implement and test a ternary search function **(15 credits)**.
...
After that write a short technical report (2-4 pages) in which you answer the questions listed in that section **(45 credits)**. To support your answers don't forget to include some runtime graphs to illustrate the behavior of the algorithms you have timed. These graphs should be produced base on the data you've collect while running your experiments.

For the tasks 1-5 should place your code [here](code/src/searching.cpp). Run the tests to make sure your functions work correctly.
...
1. Run the versions already implemented in the `<cstdlib.h>`, [`bsearch()`](http://en.cppreference.com/w/cpp/algorithm/bsearch), and in `<algorithm>`, [`binary_search()`](http://en.cppreference.com/w/cpp/algorithm/binary_search), and compare the running time of the algorithms implemented, as their input size increases linearly.
